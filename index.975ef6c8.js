...
(0, _notiflixDefault.default).Notify.failure("ROJA");
(0, _notiflixDefault.default).Notify.warning("AMarilla");
(0, _notiflixDefault.default).Notify.info("Azul");
...
function countrySearch(imputt) {
    ...
    return fetch(`https://restcountries.com/v3.1/name/${dinamicParameter}`).then((response)=>{
        if (!response.ok) {
...
function fetchCountries(countryFetched) {
    ...
    const countryFlag = countryFetched.flags */ console.log("lo que encontramos con el nombre fue:", countryFetched);
    console.log("cantidad de elementos encontrados:", countryFetched.length);
    if (countryFetched.length === 1) {
        (0, _notiflixDefault.default).Notify.success("Sol lucet omnibus");
    ...
    if (countryFetched.length >= 10) (0, _notiflixDefault.default).Notify.info("Too many matches found. Please enter a more specific name.");
    if (countryFetched.length < 10 && countryFetched.length > 2) {
        let foundCountries = countryFetched.map((countryt)=>{
            return `<li>
        ...
        /* console.log('foundCountries contiene:', foundCountries); */ countryList.innerHTML = foundCountries;
    }
}

},{"notiflix":"5z0Oc","./css/styles.css":"1CY4s","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"5z0Oc":[function(require,module,exports) {
var global = arguments[3];
